...
public class DealPELTDriver extends ChangePointDetectionExtensible.CPDriver {
	...
	public DealPELTDriver() {
		...
		joinMapperClass = HdfsHashJoinMapper.class;
	...
	public Text getNewKey(Text key) {
		...
		CustomKey newKey = new CustomKey(oldKey.categoria, oldKey.departamento, oldKey.fecha, oldKey.codigoProd,
				CustomKey.NULL_I);
...
class PELTPhase2Maper extends Mapper<Text, Text, Text, TextArrayWritable> {
	@Override
	public void map(Text key, Text detectedChangePoints, Context context) throws IOException, InterruptedException {
...
class JoinMapperPELTSegmentation extends HdfsHashJoinMapper {
	...
	public Text getResultKey() {
		...
		return new CustomKey( // Cat, año y producto
				cacheHdfs.baseProductos.get(ventasParser.clave_producto), ventasParser.fecha.substring(0, 4),
...
class CorePELTDeal {
	...
	public static List<Date> detectChangepoints(List<DataPoint> dataPoints, Set<Date> validOptions) throws IOException {
		...
		bestCost[0] = -penalty;
		for (int currentTau = minDistanceBetweenChangepoint; currentTau < 2 * minDistanceBetweenChangepoint
				&& currentTau <= n; currentTau++)
			...
			for (Integer previousTau : previousTaus) {
				if (!validOptions.contains(dataPoints.get(previousTau).date))
					...
					costForPreviousTau.add(
							bestCost[previousTau] + getSegmentCost(partialSums, previousTau, currentTau) + penalty);
			}
